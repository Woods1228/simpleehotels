                    ...
                    <div class="card-body" id="card">
                        ...
                        <% } else { %>
                            ...
                            <table class="table">
                                ...
                                <% for (Room room : rooms) { %>
                                    <%
                                        // Find the hotel for the current room
                                        Optional<Hotel> matchingHotel = hotels.stream()
                                            .filter(h -> h.getAddress().equals(room.getRoomAddress())) // Match room address with hotel address
                                            .findFirst();

                                        // Find the hotel chain name for the matching hotel
                                        String hotelChainName = matchingHotel
                                            .flatMap(h -> hotelChains.stream()
                                                .filter(hc -> hc.getAddress().equals(h.getChainAddress())) // Match hotel chain address
                                                .findFirst()
                                            )
                                            .map(HotelChain::getName) // Get the name of the hotel chain
                                            .orElse(""); // Return empty string if no match is found
                                    %>
